...
class AudioPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        volume_control: SoftwareVolume | None = None,
    ) -> None:
    ...
    def write(self, pcm_data: bytes) -> int:
        ...
        if self.config.output.format != "S16_LE":
            raise RuntimeError(
                "TMBA-Softwarelautstärke unterstützt derzeit nur S16_LE."
            )
        return self.output_driver.write(
            self._volume_control.apply_s16le(pcm_data)
        )
