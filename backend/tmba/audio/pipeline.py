...
class AudioPipeline:
    ...
    v0.6.1-A defines ordering, validation, configuration and lifecycle. It does
    not yet route real audio samples and is not connected to AudioManager.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.output_driver = output_driver or NullOutputDriver(self.config.output)
        self._stages = list(stages) if stages is not None else self._default_stages()
    ...
    def validate(self) -> None:
        ...
        output_stages = [stage for stage in self._stages if stage.stage_type == "output"]
        if len(output_stages) != 1:
            raise ValueError("AudioPipeline requires exactly one output stage.")

        if self._stages[-1].stage_type != "output":
            raise ValueError("The output stage must be the final pipeline stage.")
    ...
    def enable_stage(self, name: str) -> None:
        ...
        if stage.stage_type == "output":
            raise ValueError("The output stage cannot be disabled or enabled manually.")
        stage.enable()

    def disable_stage(self, name: str) -> None:
        stage = self.get_stage(name)
        if stage.stage_type == "output":
            raise ValueError("The output stage cannot be disabled.")
        stage.disable()

    def status(self) -> PipelineStatus:
        stage_statuses = [asdict(stage.status()) for stage in self._stages]
        output_status = asdict(self.output_driver.status())

        return PipelineStatus(
            state=self._state,
            stage_count=len(self._stages),
            enabled_stage_count=sum(stage.enabled for stage in self._stages),
